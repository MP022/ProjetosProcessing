class gameOfLife{
  ...
  void proxPasso(){
    ...
    for(int i = 0; i < this.quantCeLin; i++){
      for(int j = 0; j < this.quantCeCol; j++){
        //calcula a quantia de vizinhos vivos da celula
        k = atualizaEstadoCelula(i, j);
        
        celulas[i][j].k = k;
        prox[i][j].k = k;
        
        //Atualiza a quantidade de comida da celula
        if(celulas[i][j].vivo){
          if(celulas[i][j].comida > 0){
            prox[i][j].comida--;
          }else{
            prox[i][j].vivo = false;
          }
          
          if(celulas[i][j].comida > 5){
            prox[i][j].vivo = false;
          }
          
          celulas[i][j].comida += k/3;
          prox[i][j].comida += k/3;
        }
        
        //verifica o estado da celula no proximo passo
        if(celulas[i][j].vivo && celulas[i][j].comida <= 0){
          if(k==3||k==2){
            prox[i][j].vivo = true;
          }else{
            prox[i][j].vivo = false;
